...
type RawClassMaterialRequestRow = {
  id: string
  status: string | null
  desired_date: string | null
  desired_period: string | null
...
export default async function ManagerPrintRequestsPage({
  ...
  const [printRequestResult, classMaterialPrintRequestResult] = await Promise.all([
    supabase
      .from('print_requests')
      .select(
        `id,
  ...
  if (printRequestResult.error) {
    console.error('[manager-print-requests] print request error', printRequestResult.error)
  }

  if (classMaterialPrintRequestResult.error) {
    console.error('[manager-print-requests] class material print request error', classMaterialPrintRequestResult.error)
  }
  ...
  return (
    <section className="space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold text-slate-900">인쇄 요청 관리</h1>
        <p className="text-sm text-slate-600">
          {profile?.name ?? profile?.email} 님, 주간 인쇄 요청을 확인하고 처리 상태를 업데이트하세요.
        </p>
      </div>
