...
export default async function ManagerAbsencesPage(props: {
  ...
  const previousHref = buildWeekHref('/dashboard/manager/absences', searchParams ?? {}, weekRange.previousStart)
  const nextHref = buildWeekHref('/dashboard/manager/absences', searchParams ?? {}, weekRange.nextStart)
  ...
  if (classError) {
    console.error('[manager-absences] failed to load classes', classError)
  }

  const classes = (classRows ?? []).map((row: ClassRow) => ({
    id: row.id,
    name: row.name ?? '이름 미정',
  }))
  ...
  if (classIds.length > 0) {
    ...
    if (studentError) {
      console.error('[manager-absences] failed to load students', studentError)
    }
  ...
  if (classIds.length > 0) {
    ...
    if (reportError) {
      console.error('[manager-absences] failed to load reports', reportError)
    }
  ...
  const roleForClient = profile.role === 'principal' ? 'principal' : 'manager'

  return (
    <section className="space-y-6">
      <DashboardBackLink fallbackHref="/dashboard/manager" label="실장 대시보드로 돌아가기" />

      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold text-slate-900">결석계 관리</h1>
          <p className="text-sm text-slate-600">반별 결석계를 작성하고 교사 조치 내용을 확인하세요.</p>
        </div>
