...
import { deleteEssayPost, setEssayPostFeatured, setEssayPostHidden } from '@/lib/essay-posts'
import {
  addEssayExcellentMonth,
  fetchEssayExcellentMonths,
  selectEssayExcellentPost,
  removeEssayExcellentPost,
  type EssayExcellentMonth,
...
export async function toggleEssayHidden(input: z.infer<typeof toggleHiddenSchema>) {
  ...
  const result = await setEssayPostHidden({
...
export async function toggleEssayFeatured(input: z.infer<typeof toggleFeaturedSchema>) {
  ...
  const result = await setEssayPostFeatured({
    postId: parsed.data.postId,
...
export async function removeEssayPost(input: z.infer<typeof deleteSchema>) {
  ...
  const result = await deleteEssayPost({
    postId: parsed.data.postId,
    teacherId: profile.id,
  })
...
export async function getEssayAttachmentDownload(input: z.infer<typeof downloadAttachmentSchema>) {
  ...
  if (postError) {
    console.error('[essay] failed to load post for attachment download', postError)
  ...
  if (!attachmentFound) {
    ...
    if (attachmentError) {
      console.error('[essay] failed to verify attachment ownership', attachmentError)
    }
  ...
  if (assetError) {
    console.error('[essay] failed to load asset for download', assetError)
  ...
  try {
    ...
    if (signedError || !signed?.signedUrl) {
      console.error('[essay] failed to create signed download url', signedError, {
        bucket: bucketId,
        path: assetRow.path,
      })
    ...
    console.error('[essay] unexpected signed url error', error)
...
export async function selectEssayAsExcellent(input: z.infer<typeof selectExcellentSchema>) {
  ...
  const result = await selectEssayExcellentPost(parsed.data.monthId, parsed.data.postId, profile.id)
  if (result.success) {
    revalidateEssayPaths()
  }
  return result
...
export async function removeEssayFromExcellent(input: z.infer<typeof removeExcellentSchema>) {
  ...
  const result = await removeEssayExcellentPost(parsed.data.monthId, parsed.data.postId)
  if (result.success) {
    revalidateEssayPaths()
  }
  return result
...
export async function createEssayExcellentMonthAction(
  ...
  const result = await addEssayExcellentMonth(
    parsed.data.label,
    parsed.data.year,
    parsed.data.month,
    profile.id
...
export async function fetchEssayExcellentMonthsAction(): Promise<EssayExcellentMonth[]> {
  const { profile } = await getAuthContext()
  if (!profile) {
    return []
  }
  return fetchEssayExcellentMonths()
}
