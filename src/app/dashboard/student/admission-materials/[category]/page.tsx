...
export default async function StudentAdmissionMaterialCategoryPage({
  params,
  searchParams,
}: {
  ...
  const universityParam =
    typeof resolvedSearchParams?.university === 'string' ? resolvedSearchParams.university.trim() : ''
  ...
  if (query) {
    postQuery = postQuery.or(
      [
        ...
        `past_exam_university.ilike.%${query}%`,
      ].join(',')
    )
  }
  ...
  if (error) {
    console.error('[student-admission-materials] failed to load posts', error)
  ...
  const posts: AdmissionMaterialPostView[] = await Promise.all(
    normalizedPosts.map(async (post) => {
      ...
      if (resourcePath) {
        ...
        if (signedError) {
          console.error('[student-admission-materials] failed to sign resource url', signedError)
        } else {
  ...
  const hasFilter = Boolean(query || yearParam || universityParam || selectedAdmissionTypes.length > 0)
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {hasFilter ? (
            <Button type="button" variant="ghost" asChild>
              <Link href={`/dashboard/student/admission-materials/${category}`}>필터 초기화</Link>
            </Button>
          ) : null}
        </div>
      </form>
      ...
      {posts.length === 0 ? (
        <Card className="border-dashed border-slate-200 bg-slate-50">
          <CardContent className="py-12 text-center text-sm text-slate-500">
            {hasFilter ? '조건에 맞는 자료가 없습니다.' : '아직 등록된 자료가 없습니다.'}
          </CardContent>
        </Card>
      ) : (
                ...
                {posts.map((post) => {
                  ...
                  return (
                    <TableRow key={post.id} className="align-top">
                      ...
                      <TableCell>
                        <Link
                          href={`/dashboard/student/admission-materials/${category}/${post.id}`}
                      ...
                      <TableCell className="text-right">
                        <Button asChild size="sm" variant="secondary">
                          <Link href={`/dashboard/student/admission-materials/${category}/${post.id}`}>
                            상세 보기
                          </Link>
                        </Button>
                      </TableCell>
