...
export const metadata: Metadata = {
  title: '입시 자료 | 대학준비',
  description: '대학별 영화과 수시 일정과 모집단위 산식·컷, 기출·합격 복기 자료를 열람합니다.',
}

const STUDENT_UNIVERSITY_BASE = '/dashboard/student/admission-materials/universities'

const VISIBLE_CATEGORIES: AdmissionMaterialCategory[] = ['past_exam']

export default async function StudentAdmissionMaterialsPage() {
    ...
    <section className="space-y-6">
      <div className="space-y-3">
        ...
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold text-slate-900">입시 자료</h1>
          <p className="text-sm text-slate-600">
            대학별 영화과 수시 일정과 모집단위 산식·컷을 확인하고, 기출·합격 복기 자료를 열람하세요.
          </p>
        </div>
      </div>
      ...
      <section className="space-y-3">
        ...
        <UniversityScheduleCalendar
          ...
          detailBasePath={STUDENT_UNIVERSITY_BASE}
        />
      </section>
      ...
      <Card className="border-slate-200 shadow-sm">
        ...
        <CardContent className="p-0">
          <UniversityList
            ...
            basePath={STUDENT_UNIVERSITY_BASE}
          />
        </CardContent>
      </Card>
      ...
      <section className="space-y-3">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500">카테고리를 선택해 대학별 기출 자료를 열람하세요.</p>
        </div>
              ...
              <Card key={category} className="border-slate-200 shadow-sm transition hover:-translate-y-1 hover:shadow-md">
                ...
                <CardContent>
                  <Button asChild className="w-full">
                    <Link href={`/dashboard/student/admission-materials/${category}`}>카테고리 열기</Link>
                  </Button>
                </CardContent>
              </Card>
            )
      ...
      <Card className="border-slate-200 bg-slate-50 shadow-sm">
        <CardHeader className="space-y-2">
          ...
          <CardDescription className="text-sm text-slate-500">
            합격생들의 면접·실기·글쓰기 복기를 대학·학년도·학생 이름으로 분류해 열람합니다.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild className="w-full sm:w-auto">
            <Link href="/dashboard/student/admission-reviews">복기 아카이브 열기</Link>
          </Button>
        </CardContent>
      </Card>
    </section>
