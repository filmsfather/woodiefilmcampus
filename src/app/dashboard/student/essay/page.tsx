...
import { fetchEssayExcellentPostsByMonth } from '@/lib/essay-excellent'
...
interface StudentEssayPageProps {
...
export default async function StudentEssayPage(props: StudentEssayPageProps) {
  ...
  return (
    <section className="flex flex-col gap-6">
      <header className="space-y-2">
        <h1 className="text-2xl font-semibold text-slate-900">학생 에세이 보드</h1>
        <p className="text-sm text-slate-600">
          모든 학생이 제출한 에세이 PDF를 한곳에서 살펴보고 영감을 나눠보세요. 내 제출물은 필요에 따라 숨길 수 있습니다.
        </p>
      </header>

      <EssayFiltersForm
        basePath="/dashboard/student/essay"
      ...
      <EssayPostList items={data.items} viewerId={profile.id} viewerRole={profile.role} />
