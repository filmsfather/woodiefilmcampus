...
export interface EssayExcellentMonth {
  id: string
  label: string
  year: number
  month: number
...
export interface EssayExcellentPostItem {
  postId: string
  studentName: string
...
export interface EssayExcellentMonthGroup {
  month: EssayExcellentMonth
  posts: EssayExcellentPostItem[]
}

export interface EssayPostExcellenceEntry {
  monthId: string
  monthLabel: string
}

export async function fetchEssayExcellentMonths(): Promise<EssayExcellentMonth[]> {
  const admin = createAdminClient()

  const { data, error } = await admin
    .from('essay_excellent_months')
    .select('id, label, year, month')
    .order('year', { ascending: false })
    .order('month', { ascending: false })

  if (error) {
    console.error('[essay-excellent] failed to fetch months', error)
    return []
  }
...
export async function addEssayExcellentMonth(
  label: string,
  year: number,
  month: number,
  createdBy: string
): Promise<{ success: true; month: EssayExcellentMonth } | { success: false; error: string }> {
  const admin = createAdminClient()
  ...
  if (error) {
    ...
    console.error('[essay-excellent] failed to add month', error)
...
export async function selectEssayExcellentPost(
  monthId: string,
  postId: string,
  selectedBy: string
  ...
  if (error) {
    console.error('[essay-excellent] failed to select post', error)
...
export async function removeEssayExcellentPost(
  monthId: string,
  postId: string
): Promise<{ success: true } | { success: false; error: string }> {
  const admin = createAdminClient()
  ...
  if (error) {
    console.error('[essay-excellent] failed to remove excellent post', error)
...
export async function getEssayPostExcellenceMap(
  postIds: string[]
): Promise<Map<string, EssayPostExcellenceEntry>> {
  if (postIds.length === 0) {
    return new Map()
  }
  ...
  const { data, error } = await admin
    .from('essay_excellent_posts')
    .select('post_id, month_id, essay_excellent_months(id, label)')
    .in('post_id', postIds)

  if (error) {
    console.error('[essay-excellent] failed to load excellence map', error)
    return new Map()
  }

  const map = new Map<string, EssayPostExcellenceEntry>()

  for (const row of data ?? []) {
    const postId = row.post_id as string
    const monthRelation = pickFirstRelation<{ id: string; label: string }>(row.essay_excellent_months)
...
export async function fetchEssayExcellentPostsByMonth(): Promise<EssayExcellentMonthGroup[]> {
  const admin = createAdminClient()
  ...
  if (monthsError || !months || months.length === 0) {
    if (monthsError) {
      console.error('[essay-excellent] failed to fetch months', monthsError)
    }
    return []
  }
  ...
  const { data: excellentRows, error: excellentError } = await admin
    ...
    .select(`
      ...
      essay_posts!inner(
        ...
        workbooks:workbooks!essay_posts_workbook_id_fkey(id, title),
  ...
  if (excellentError) {
    console.error('[essay-excellent] failed to fetch excellent posts', excellentError)
    return []
  }
  ...
  for (const row of rows) {
    const post = row.essay_posts as JsonRecord | null
    if (!post) continue
    if (typeof post.media_asset_id === 'string') {
      mediaAssetIds.add(post.media_asset_id)
    }
    const assets = Array.isArray(post.essay_post_assets) ? post.essay_post_assets : []
  ...
  if (assetIdArray.length > 0) {
    ...
    if (assetError) {
      console.error('[essay-excellent] failed to load media assets', assetError)
    } else {
  ...
  const monthMap = new Map<string, EssayExcellentMonthGroup>()
  ...
  for (const row of rows) {
    ...
    const rawAttachments = Array.isArray(post.essay_post_assets) ? post.essay_post_assets : []
