...
export const FORMULA_PRESETS: Readonly<Record<string, FormulaPreset>> = {
  ...
  'dongguk-2027-hakjong-chuchon-film': {
    ...
    spec: {
      ...
      subjectGroupTopK: [
        {
          label: '국·수·사·영·한국사 상위 10과목',
          subjects: ['국어', '수학', '사회', '영어', '한국사'],
          topK: 10,
        },
      ],
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '학교장추천 인문계열 산식. 국·수·사·영·한국사 석차등급 상위 10과목 단순평균 → 700점 환산. ' +
  ...
  'soongsil-2027-yechaeneung-film-direction': {
    ...
    spec: {
      ...
      weights: { ...NO_CAREER_WEIGHTS },
      totalScore: 100,
      subjectAreaWeights: { 국어: 35, 수학: 15, 영어: 35, 사회: 15 },
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '교과영역별 가중치(국 35 / 수 15 / 영 35 / 사(한국사 포함) 15) 적용. ' +
        '각 영역 이수단위 가중평균을 구한 뒤 영역 가중치로 가중합(한국사는 사회 교과군에 합산). ' +
        '진로선택 미반영. 1단계 학생부 40%, 2단계 실기 100%.',
    },
  ...
  'kookmin-2027-silgi-film': {
    ...
    spec: {
      ...
      subjectGroupTopK: [
        {
          label: '국·영 공통/일반선택(전과목)',
          subjects: ['국어', '영어'],
          courseTypes: ['공통', '일반선택'],
          topK: 999,
        },
        {
          label: '예술·체육 진로선택 상위 3',
          subjects: ['예술', '체육'],
          courseTypes: ['진로선택'],
          topK: 3,
        },
      ...
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '국·영 공통/일반선택(석차등급) 전과목 85% + 예술/체육 진로선택 상위 3과목(성취도) 15%. ' +
        '국·영 진로선택, 예·체 공통/일반선택은 반영에서 제외(그룹 정의로 분리). ' +
  ...
  'seokyeong-2027-silgi-film': {
    ...
    spec: {
      ...
      totalScore: 1000,
      subjectGroupTopK: [
        { label: '국어 상위 3', subjects: ['국어'], topK: 3 },
        { label: '영어 상위 3', subjects: ['영어'], topK: 3 },
        { label: '수학 상위 3', subjects: ['수학'], topK: 3 },
        { label: '한국사 상위 3', subjects: ['한국사'], topK: 3 },
      ],
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
  ...
  'joongbu-2027-silgi-film-direction': {
    ...
    spec: {
      ...
      subjectGroupTopK: [
        {
          label: '국·영·수·사(한국사)·과 상위 10과목',
          subjects: ['국어', '영어', '수학', '사회', '한국사', '과학'],
          topK: 10,
        },
      ],
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '국·영·수·사(한국사 포함)·과 중 상위 10과목(진로선택 포함) 평균등급 → 16단계 표 환산(200~120점). ' +
        '본 시스템은 등급평균으로 근사 비교. 실기 80%, 교과 20%.',
    },
  ...
  'suwon-2027-silgi-film': {
    ...
    spec: {
      ...
      totalScore: 300,
      subjectGroupTopK: [
        { label: '국어 상위 5', subjects: ['국어'], topK: 5 },
        { label: '수학 상위 5', subjects: ['수학'], topK: 5 },
        { label: '영어 상위 5', subjects: ['영어'], topK: 5 },
        { label: '탐구(사·과) 상위 5', subjects: ['사회', '과학'], topK: 5 },
      ],
      topGroups: 2,
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '국·수·영·탐구(사 또는 과) 중 평균등급이 좋은 상위 2개 교과영역만 반영(영역별 상위 5과목). ' +
  ...
  'yongin-2027-silgi-film-general': {
    ...
    spec: {
      ...
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '실기학과 30% 반영 등급점수(1등급 150 ~ 9등급 0). 국·영·수·사·과 중 학년별 상위 3과목 × 3학년 = 9과목 단순평균(이수단위 미적용). ' +
        '실기 70%(350/500점).',
    },
  ...
  'yongin-2027-silgi-film-opportunity': {
    ...
    spec: {
      ...
      totalScore: 150,
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes: '용인대 일반학생과 동일 산식(기회균형 특별전형). 학년별 상위 3과목 × 3학년 단순평균(이수단위 미적용).',
    },
  ...
  'daejin-2026-silgi-film': {
    ...
    spec: {
      ...
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '국·영·수·사(한국사)·과 중 상위 15과목(진로선택 최대 5과목) 등급평균 → 30단계 환산표 적용(원문). ' +
        '본 시스템은 단순 등급평균으로 근사. 2026학년도 기준이므로 2027 발표 시 갱신 필요.',
    },
  ...
  'pyeongtaek-2027-ptu-gyoga-film': {
    ...
    spec: {
      ...
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        'PTU교과 산식. 국·영·수·사·과·한국사 중 상위 9과목(진로선택 최대 3) 단순평균(이수단위 미적용) → 1등급 1000점 ~ 9등급 0점. ' +
  ...
  'kyungsung-2027-silgi-film': {
    ...
    spec: {
      ...
      totalScore: 100,
      subjectGroupTopK: [
        { label: '국어', subjects: ['국어'], courseTypes: ['공통', '일반선택'], topK: 2 },
        { label: '수학', subjects: ['수학'], courseTypes: ['공통', '일반선택'], topK: 2 },
        { label: '영어', subjects: ['영어'], courseTypes: ['공통', '일반선택'], topK: 2 },
        {
          label: '탐구(한국사 포함)',
          subjects: ['사회', '과학', '한국사'],
          courseTypes: ['공통', '일반선택'],
      ...
      notes:
        ...
        '국·수·영·탐구(한국사 포함) 공통/일반선택 각 2과목(8과목) + 진로선택(국수영탐) 상위 2과목 = 총 10과목. ' +
        '진로선택 성취도 A=2·B=4·C=6 환산.',
    },
  ...
  'kyungsung-2027-gyoga-film': {
    ...
    spec: {
      ...
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '일반계고교과 교과 100% 반영(총점 1000, 840~1000). ' +
        '국·수·영·탐구(한국사 포함) 공통/일반선택 각 2과목(8과목) + 진로선택(국수영탐) 상위 2과목 = 총 10과목. ' +
        '진로선택 성취도 A=2·B=4·C=6 환산. 등급차 2점 × 10 배수.',
    },
  ...
  'dongseo-2027-silgi-film-series1': {
    ...
    spec: {
      ...
      totalScore: 200,
      subjectGroupTopK: [
        { label: '국어(최대 3)', subjects: ['국어'], courseTypes: ['공통', '일반선택'], topK: 3 },
        { label: '영어(최대 3)', subjects: ['영어'], courseTypes: ['공통', '일반선택'], topK: 3 },
        { label: '수학(최대 3)', subjects: ['수학'], courseTypes: ['공통', '일반선택'], topK: 3 },
        { label: '사회(최대 3)', subjects: ['사회', '한국사'], courseTypes: ['공통', '일반선택'], topK: 3 },
        { label: '과학(최대 3)', subjects: ['과학'], courseTypes: ['공통', '일반선택'], topK: 3 },
        {
          label: '진로선택(최대 2)',
      ...
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '계열1(영화·뮤지컬·연기) 통합 실기전형. 국·영·수·사·과 각 최대 3과목 + 진로선택 최대 2과목 중 전체 상위 10과목 환산점수 합산 → 학생부 20%(200점 만점). 실질 9.1%.',
    },
  ...
  'cheongju-2027-yechaeneung-film': {
    ...
    spec: {
      ...
      subjectGroupTopK: [
        {
          label: '국·영·수·사·과·제2외국어/한문 상위 8과목',
          subjects: ['국어', '영어', '수학', '사회', '과학', '제2외국어', '한문'],
          topK: 8,
        },
      ],
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        ...
        '국·영·수·사·과·제2외국어/한문 중 상위 8과목 등급평균으로 근사 비교. 교과 20%, 실기 80%.',
    },
  ...
  'seoularts-2026-ilban-film': {
    ...
    spec: {
      ...
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '학생부 200점(실질 16.0%, 등급 간 19점). 국·영 석차등급 단순평균(이수단위 미적용). ' +
        '실기 800점(작문+이미지분석+구두문답) 합산 1000점. 2026 기준이므로 2027 발표 시 갱신 필요.',
    },
  ...
  'sungkyul-2027-silgi-film': {
    ...
    spec: {
      ...
      outputs: ['grade_mean_with_career', 'converted_score_1000'],
      notes:
        '석차등급 상위 12과목(진로선택 최대 2과목 포함) 이수단위 가중평균. 등급별 환산점수(1등급 5 ~ 9등급 1)를 ' +
