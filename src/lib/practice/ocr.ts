...
/**
 * 제출된 원고 사진들을 storage에서 내려받아 Gemini로 전사하고 응시에 반영합니다.
 * 실패해도 제출 자체에는 영향을 주지 않도록 ocr_status만 갱신합니다.
 * 피드백 화면은 ocr_status가 done이 아니면 원본 사진을 먼저 보여줍니다.
 */
export async function runPracticeOcrForAttempt(attemptId: string): Promise<{ success: boolean; error?: string }> {
  const admin = createAdminClient()

  await admin.from('practice_attempts').update({ ocr_status: 'processing' }).eq('id', attemptId)

  const markFailed = async (message: string) => {
    await admin.from('practice_attempts').update({ ocr_status: 'failed' }).eq('id', attemptId)
  ...
  type Row = {
    order_index: number
    media_assets:
      | { bucket: string | null; path: string | null; mime_type: string | null }
      | { bucket: string | null; path: string | null; mime_type: string | null }[]
      | null
  }
  ...
  if (error || !data || data.length === 0) {
    if (error) console.error('[practice] failed to load submission assets for ocr', error)
  ...
  for (const row of data as unknown as Row[]) {
    ...
    if (downloadError || !blob) {
      console.error('[practice] failed to download submission image for ocr', downloadError)
  ...
  if ('error' in result) {
    console.error('[practice] ocr failed', result.error)
  ...
  if (updateError) {
    console.error('[practice] failed to save ocr text', updateError)
