...
import { completePracticeRecordingAction } from '@/app/dashboard/teacher/practice-feedback/actions'
...
/** 면접 표준 길이. 초과하면 안내만 하고 강제 종료하지는 않는다. */
const TARGET_SECONDS = 5 * 60

// 용량 최소화 설정: 480p / 15fps / VP9 0.5Mbps (5분 녹화 기준 약 20MB)
...
interface PracticeInterviewRecorderProps {
  attemptId: string
  studentName: string
  uploaderId: string
...
export function PracticeInterviewRecorder({
  attemptId,
  studentName,
  uploaderId,
  hasExistingRecording,
}: PracticeInterviewRecorderProps) {
  ...
  const handleEnableCamera = async () => {
    ...
    try {
      ...
      console.error('[practice] failed to open webcam', err)
  ...
  const uploadRecording = useCallback(
    async (blob: Blob, mimeType: string) => {
      ...
      try {
        ...
        const fileName = `practice-${attemptId}.${extension}`
        ...
        const result = await completePracticeRecordingAction({
          attemptId,
          video: {
            bucket: PRACTICE_RECORDINGS_BUCKET,
            path: uploaded.path,
        ...
        console.error('[practice] failed to upload recording', err)
        setError(err instanceof Error ? err.message : '영상 업로드에 실패했습니다.')
        setPhase('ready')
      }
    },
    [attemptId, router, uploaderId]
  )
  ...
  const handleStartRecording = () => {
    ...
    try {
      ...
      console.error('[practice] failed to create MediaRecorder', err)
    ...
    <Card className="border-slate-200">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base text-slate-900">
          ...
          {studentName} 학생 면접 {hasExistingRecording ? '다시 녹화' : '녹화'}
        </CardTitle>
        <p className="text-xs text-slate-500">
          480p / 15fps / 0.5Mbps로 녹화됩니다. 5분 면접을 진행한 뒤 녹화를 종료하면 영상이 업로드되고, 남은 시간에
          피드백을 작성합니다.
          {hasExistingRecording ? ' 새로 녹화하면 기존 영상은 교체됩니다.' : ''}
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="relative overflow-hidden rounded-lg border border-slate-200 bg-slate-950">
          ...
          {phase === 'recording' && (
            <div
              className={[
                'absolute left-3 top-3 flex items-center gap-2 rounded-full px-3 py-1 text-xs font-medium text-white',
                overTarget ? 'bg-amber-600/90' : 'bg-red-600/90',
              ].join(' ')}
            >
              <span className="h-2 w-2 animate-pulse rounded-full bg-white" />
              REC {formatElapsed(elapsedSeconds)} / {formatElapsed(TARGET_SECONDS)}
            </div>
          )}
          {phase === 'uploading' && (
