...
export function AdmissionMaterialPostForm({
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const schedulePayload = schedules.map((item) => ({
      ...
      startAt: item.startAt,
      endAt: item.endAt,
      location: item.location.trim().length > 0 ? item.location.trim() : undefined,
      ...
      <CardContent className="space-y-4">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-2">
            <Label htmlFor="targetLevel">준비 대상 (선택)</Label>
            <Input
              id="targetLevel"
              name="targetLevel"
              placeholder="예: 영화과 3학년, 수시 준비반"
              defaultValue={defaults?.targetLevel ?? ''}
              maxLength={120}
              disabled={isPending}
            />
            <p className="text-xs text-slate-500">자료가 어떤 학생을 위한 것인지 표시하면 검색이 쉬워집니다.</p>
          </div>
          ...
          <div className="grid gap-2">
            <Label htmlFor="description">자료 설명 (선택)</Label>
            <Textarea
              id="description"
              name="description"
              rows={5}
              defaultValue={defaults?.description ?? ''}
              placeholder="자료 활용법이나 참고 사항을 간단히 작성하세요."
              disabled={isPending}
            />
          </div>
          ...
          <div className="grid gap-2">
            ...
            <p className="text-xs text-slate-500">업로드 제한: 최대 {maxSizeLabel}. 새 파일을 선택하면 기존 파일을 교체합니다.</p>
          </div>
                  ...
                  <div key={schedule.id} className="rounded-md border border-slate-200 p-4">
                    ...
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                      <div className="grid gap-2">
                        ...
                        <Input
                          ...
                          placeholder="예: 중앙대 모의 면접"
                          maxLength={150}
                          disabled={isPending}
                        />
                      </div>
                      <div className="grid gap-2">
                        <Label className="text-xs text-slate-500">장소 (선택)</Label>
                        <Input
                          value={schedule.location}
                          onChange={(event) => updateSchedule(schedule.id, 'location', event.target.value)}
                          placeholder="예: 캠퍼스 3층 세미나실"
                          maxLength={120}
                          disabled={isPending}
                        />
                      </div>
                    ...
                    <div className="mt-3 grid gap-2">
                      ...
                      <Textarea
                        ...
                        rows={3}
                        placeholder="지참 서류나 사전 준비 사항을 적어두세요."
                        disabled={isPending}
                      />
                    </div>
                  </div>
