...
import { createWritingSessionAction } from '@/app/dashboard/teacher/mock-practice/writing/actions'
...
export interface WritingClassOption {
  id: string
  name: string
  students: Array<{ id: string; name: string }>
}

interface WritingSessionCreateFormProps {
  setId: string
  classOptions: WritingClassOption[]
}

export function WritingSessionCreateForm({ setId, classOptions }: WritingSessionCreateFormProps) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await createWritingSessionAction({
        setId,
      ...
      if (result.success) {
        router.push(`/dashboard/teacher/mock-practice/writing/sessions/${result.id}`)
        router.refresh()
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
        총 <span className="font-semibold text-slate-900">{totalStudents}</span>명의 학생에게 출제됩니다. 문제는
        학생이 시험 시작을 누른 순간에 공개되고, 그때부터 제한시간이 시작됩니다.
      </div>
